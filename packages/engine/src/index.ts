...
function extractCategory(ruleId: string): string {
  if (ruleId.startsWith('ETHICAL')) return 'ethical';
  if (ruleId.startsWith('AI-AUDIT')) return 'ai-audit';
  if (ruleId.startsWith('AU-SBD')) return 'au-sbd';
  if (ruleId.startsWith('AU-OSA')) return 'au-osa';
  if (ruleId.startsWith('caadca')) return 'caadca';
  if (ruleId.startsWith('AI-RISK')) return 'ai-risk';
  if (ruleId.startsWith('AI-TRANSPARENCY')) return 'ai-transparency';
  if (ruleId.startsWith('AI-GOVERNANCE')) return 'ai-governance';
  if (ruleId.startsWith('AI-OVERSIGHT')) return 'ai-oversight';
  if (ruleId.startsWith('AI-ACCURACY')) return 'ai-accuracy';
  if (ruleId.startsWith('CAI-')) return 'constitutional-ai';
...
const REMEDIATION_MAP: Record<string, RemediationSpec> = {
  ...
  // Tier 3 — Flag only (ethical design, requires human judgment)
  'ETHICAL-001':      { fixability: 'flag-only', guidanceUrl: 'https://docs.runhalo.dev/ethical/infinite-scroll', estimatedCost: '$0' },
  'ETHICAL-002':      { fixability: 'flag-only', guidanceUrl: 'https://docs.runhalo.dev/ethical/streak-pressure', estimatedCost: '$0' },
  'ETHICAL-003':      { fixability: 'flag-only', guidanceUrl: 'https://docs.runhalo.dev/ethical/variable-rewards', estimatedCost: '$0' },
  'ETHICAL-004':      { fixability: 'flag-only', guidanceUrl: 'https://docs.runhalo.dev/ethical/manipulative-notifications', estimatedCost: '$0' },
  'ETHICAL-005':      { fixability: 'flag-only', guidanceUrl: 'https://docs.runhalo.dev/ethical/artificial-scarcity', estimatedCost: '$0' },
};
...
// Ethical Design Rules (Preview)
export const ETHICAL_RULES: Rule[] = [
...
export const AI_AUDIT_RULES: Rule[] = [
...
export const AU_SBD_RULES: Rule[] = [
  ...
  {
    id: 'AU-SBD-005',
    is_active: false,  //W1: 18.2% precision (2/11 TP). All 9 FPs are media player autoplay (jellyfin). Pattern too broad — rebuild needed.
...
export interface EngineConfig {
  ...
  ethical?: boolean; // Enable ethical design rules (experimental)
  aiAudit?: boolean; // Enable AI-generated code audit rules
  sectorAuSbd?: boolean; // Enable AU Safety by Design sector rules (P3-3)
  sectorAuOsa?: boolean; // Enable AU Online Safety Act rules
  packs?: string[];        // Pack IDs to load from rules.json (e.g. ['coppa', 'ethical'])
...
export class HaloEngine {
  ...
  static resolvePacks(config: EngineConfig): string[] {
    ...
    if (config.ethical || config.aiAudit || config.sectorAuSbd || config.sectorAuOsa) {
      const packs = ['coppa'];
      if (config.ethical) packs.push('ethical');
      if (config.aiAudit) packs.push('ai-audit');
      if (config.sectorAuSbd) packs.push('au-sbd');
      if (config.sectorAuOsa) packs.push('au-osa');
      return packs;
    }
  ...
  scanFile(filePath: string, content: string): Violation[] {
    ...
    if (govViolations.size > 0) {
      violations = violations.filter(v =>
        !(v.ruleId === 'AI-RISK-003' && govViolations.has(`${v.filePath}:${v.line}`))
      );
    }

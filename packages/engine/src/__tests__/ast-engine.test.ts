...
describe('ASTRuleEngine', () => {
  ...
  // =========================================================================
  // 7. ETHICAL-001 — Infinite Scroll
  // =========================================================================

  describe('ETHICAL-001: Infinite Scroll', () => {
    const ruleId = 'ETHICAL-001';

    it('should suppress when IntersectionObserver is used for lazyLoad', () => {
      if (!isTreeSitterAvailable()) return;
      ...
      expect(result.verdict).toBe('suppressed');
      expect(result.confidence).toBeGreaterThanOrEqual(0.80);
      expect(result.reason).toMatch(/lazy.*load/i);
    });

    it('should confirm when IntersectionObserver is used with loadMore', () => {
      if (!isTreeSitterAvailable()) return;
      ...
      const tree = parseTS(code);
      // Violation at the IntersectionObserver creation line
      const violation = makeViolation(ruleId, code, 2);
      ...
      expect(result.verdict).toBe('confirmed');
      expect(result.confidence).toBeGreaterThanOrEqual(0.75);
      expect(result.reason).toMatch(/infinite.*scroll|stopping/i);
    });

    it('should confirm when IntersectionObserver is used with fetchNext', () => {
      if (!isTreeSitterAvailable()) return;
    ...
    it('should suppress when context has lazy image loading keywords', () => {
      if (!isTreeSitterAvailable()) return;
      ...
      // Context has "img" and "src" which match lazy loading pattern
      expect(result.verdict).toBe('suppressed');
      expect(result.confidence).toBeGreaterThanOrEqual(0.75);
    });

    it('should suppress when maxPages limit exists in scope', () => {
      if (!isTreeSitterAvailable()) return;
      ...
      expect(result.verdict).toBe('suppressed');
      expect(result.confidence).toBeGreaterThanOrEqual(0.70);
      expect(result.reason).toMatch(/max|limit/i);
    });
  });

  // =========================================================================
  // 8. Type definition file suppression
  // =========================================================================

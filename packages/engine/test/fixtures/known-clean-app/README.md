...
When Halo scans this directory with all rule packs enabled (COPPA, Ethical,
AI-Audit, AU-SBD), it must report **zero violations**. Any scan that returns
one or more violations is a false-positive bug in the engine.
...
|------|---------------------|----------------------|
| `src/auth.ts` | Age gate before auth, parental consent flow, first-party login only | `coppa-auth-001`, `coppa-data-002`, `coppa-flow-009`, `coppa-default-020`, `AI-AUDIT-002`, `coppa-sec-006` |
| `src/data.ts` | AES-256-GCM encrypted PII, `deleted_at` / `expiresAt` retention sentinels, no plaintext storage | `coppa-retention-005`, `coppa-cookies-016`, `coppa-data-002`, `coppa-default-020`, `AI-AUDIT-002`, `AU-SBD-001` |
| `src/tracking.ts` | First-party only analytics, no SDK initialisation, no `identify()` with PII | `coppa-tracking-003`, `coppa-analytics-018`, `coppa-geo-004`, `AI-AUDIT-001`, `AI-AUDIT-004`, `coppa-cookies-016` |
| `src/ui.ts` | Safe text rendering, external-link overlay, pagination (no infinite scroll), effort-based rewards, cumulative progress | `coppa-sec-015`, `coppa-ugc-014`, `coppa-notif-013`, `coppa-ext-017`, `coppa-bio-012`, `ETHICAL-001` through `ETHICAL-005`, `AU-SBD-005` |
| `src/cookies.ts` | Secure cookie attribute builder, non-PII preference storage, consent record, notification gate | `coppa-cookies-016`, `AI-AUDIT-005`, `coppa-notif-013` |
...
Rules avoided: **AI-AUDIT-002** (`Hardcoded Secrets`), general PII hygiene.
...
Rules avoided: **coppa-tracking-003**, **AI-AUDIT-001**, **AI-AUDIT-004**.
...
Rules avoided: **coppa-geo-004**, **AU-SBD-006**.
...
`sameSite: 'none'` combined with `secure: false` never occurs (the combination
flagged by **AI-AUDIT-005**).
...
Rules avoided: **coppa-cookies-016**, **AI-AUDIT-005**.
...
| Dark Pattern | Rule | What this fixture uses instead |
|---|---|---|
| Infinite scroll | ETHICAL-001 | Explicit `buildPaginationControls()` with Previous/Next buttons |
| Streak pressure | ETHICAL-002 | `renderProgressSummary()` shows cumulative `totalDaysActive` |
| Loot boxes / random rewards | ETHICAL-003 | `renderEarnedReward()` — deterministic, effort-based only |
| FOMO language | ETHICAL-004 | Neutral, informational copy throughout |
| Countdown timers | ETHICAL-005 | No time-pressure UI elements |
| Autoplay | AU-SBD-005 | No `autoplay: true` setting anywhere |
...
Rules avoided: **coppa-default-020**, **AU-SBD-001**.
...
Rules avoided: **AI-AUDIT-005**, **coppa-sec-010**, **AI-AUDIT-002**.
...
describe('known-clean-app fixture', () => {
  const engine = new HaloEngine({ ethical: true, aiAudit: true, sectorAuSbd: true });

...
function formatText(results: ScanResult[], verbose: boolean = false, fileCount: number = 0, scoreResult?: any): string {
  ...
  header += `\n${c(colors.dim, '📋 26 COPPA rules. Upgrade for 13+ jurisdictions: runhalo.dev/pricing')}\n`;
...
function generatePdfReport(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (reviewData && reviewData.results.length > 0) {
      ...
      doc.fontSize(7).fillColor(PDF_COLORS.lightText);
      doc.text(
        `Reviewed by Halo AI Review Board in ${reviewData.latency_ms}ms. Cost: $${reviewData.cost.estimated_usd.toFixed(4)}. ${reviewData.summary.cache_hits} results served from cache.`,
    ...
    if (isBusiness) {
      ...
      const methodologyItems = [
        ...
        ['Rules Version', '26 COPPA rules (free tier). Pro: 180+ rules across 13 jurisdictions'],
        ['Analysis Tiers', 'Tier 1 (Regex) + Tier 2 (AST) + Tier 3 (AI Review Board)'],
        ['AI Model', 'LLM-powered compliance reasoning'],
      ...
      doc.fontSize(10).fillColor(PDF_COLORS.bodyText);
      doc.text(
        'This scan was performed using Halo\'s three-tier detection pipeline. Tier 1 identifies potential compliance issues using pattern-based analysis across 26 COPPA rules in free tier. Tier 2 enriches findings with structural code analysis (AST, data-flow tracing, import graph). Tier 3 applies AI-powered compliance reasoning with regulatory intelligence and dollar risk scoring.',
        60, y, { width: pageWidth }
      );
      y += 60;
...
function resolvePacks(options: CLIOptions): string[] {
  ...
  // Legacy boolean flags — if ANY are set, use selective mode (Pro features)
  if (options.ethicalPreview || options.aiAudit) {
    const packs = ['coppa'];
  ...
  // Free tier: COPPA pack only. Pro unlocks additional packs.
  return ['coppa'];
}
...
program
  ...
  .option('--pack <packs...>', 'Rule packs to scan against (e.g., coppa)')

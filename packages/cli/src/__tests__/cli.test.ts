...
describe('Halo CLI', () => {
  ...
  describe('AU Safety by Design Sector Rules (P3-3)', () => {
    const { checkProFeature: checkPro, saveConfig: saveCfg } = require('../index');

    it('should load AU SbD rules via engine config flag', () => {
      const { AU_SBD_RULES } = require('@runhalo/engine');
      expect(AU_SBD_RULES).toHaveLength(6);
      expect(AU_SBD_RULES[0].id).toBe('AU-SBD-001');
      expect(AU_SBD_RULES[5].id).toBe('AU-SBD-006');
    });

    it('should gate --sector-au-sbd behind Pro tier', () => {
      // Simulate free tier
      saveCfg({
        prompted: true,
        promptedAt: new Date().toISOString(),
      ...
      // Suppress stderr upsell output
      const stderrWrite = process.stderr.write;
      process.stderr.write = (() => true) as any;

      delete process.env.CI;
      Object.defineProperty(process.stdout, 'isTTY', { value: true, configurable: true });
      expect(checkPro('AU Safety by Design Rules', '--sector-au-sbd')).toBe(false);

      process.stderr.write = stderrWrite;
    });

    it('should allow --sector-au-sbd for Pro tier', () => {
      saveCfg({
        prompted: true,
        promptedAt: new Date().toISOString(),
        consent: true,
      ...
      expect(checkPro('AU Safety by Design Rules', '--sector-au-sbd')).toBe(true);
    });

    it('should detect AU SbD violations when scanning', () => {
      const engine = new HaloEngine({ sectorAuSbd: true });
      const code = `const settings = { visibility: 'public', autoplay: true, shareLocation: true };`;
      const violations = engine.scanFile('settings.ts', code);
      const sbdViolations = violations.filter(v => v.ruleId.startsWith('AU-SBD'));
      expect(sbdViolations.length).toBeGreaterThan(0);
    });
  });
